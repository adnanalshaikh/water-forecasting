...
def plot_walkforward_forecast(train, test, walk_forward_forecast, intermediate_forecast, 
    ...
    # show on screen 
    if plot.get("show", False):
        plt.show()
        
    # Get plot configuration (with defaults)
    plot_cfg = config.get("plot", {})
    
    if plot_cfg.get("save", False):
        figures_dir = results_dir / f"{config.get('run_dir', 'run_xxx')}_figures"
        figures_dir.mkdir(parents=True, exist_ok=True)
    
        base_name = figures_dir / f"area_{area_id}_forecast"
    
        # Choose dpi based on mode
        mode = plot_cfg.get("mode", "low")
        dpi = plot_cfg.get("dpi", 600 if mode == "high" else 150)
    
        # Choose formats (defaults to ["png"])
        formats = plot_cfg.get("formats", ["png"])
    
        for ext in formats:
            fig.savefig(f"{base_name}.{ext}",
                        dpi=dpi,
                        bbox_inches="tight",
                        format=ext,
                        transparent=False,
