...
class ResultsOrganizer:
    """
    class for organizing and presenting forecasting results
    """
...
########### mase visulization 2 ####################
    ...
    # Standalone function for triple metrics analysis
...
def quick_results_summary(log_filepath='experiment_log.csv'):
    ...
    return organizer
def compare_two_runs_statistically(path1, path2):
    ...
    t_stat, p_value = stats.ttest_rel(run1_mape, run2_mape)
    improvement = (run1_mape.mean() - run2_mape.mean()) / run1_mape.mean() * 100
    
    print(f"run1 vs run2 features: {improvement:+.1f}% change in MAPE (p={p_value:.4f})")
...
if __name__ == "__main__":
    ...
    quick_summary('../results/run1.csv')
    compare_two_runs_statistically('../results/run1.csv', '../results/exp2_adaptive_reg.csv') 
